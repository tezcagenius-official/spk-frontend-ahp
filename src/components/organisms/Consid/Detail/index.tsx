    ...
    <div className="flex gap-3 flex-col">
      ...
      <Card className="p-3 grid grid-cols-3 gap-3">
        {dataAlt?.data?.kriteria.map((k) => (
          <div key={k.kriteria_id} className="bg-neutral-800 p-2 rounded-lg">
            <h2 className="text-xl font-semibold">
              Kriteria {k.nama_kriteria}
            </h2>
            <p>prioritas - {k.prioritas}</p>
            <div className="mt-3 grid grid-cols-3 gap-2">
              {k.sub_kriteria.map((sk, i) => (
                <div
                  key={`${i}-${sk.kriteria_id}`}
                  className="rounded-md p-2 border border-neutral-700"
                >
